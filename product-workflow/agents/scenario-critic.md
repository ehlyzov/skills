...
В каждом сценарии 9-10 разделов: Problem / Goal / Non-goals / User flow / Functional requirements / Non-functional requirements / Architecture constraints / Acceptance criteria / Test plan [+ Дополнительные сценарии и связь с продуктом, если Phase 2 уже прошла].
...
2. **Связность.** Cross-references двусторонни. Persona в шапке сценария совпадает с overview index. Goals из overview покрыты acceptance/NFR хотя бы одного сценария с тем же числом.
...
4. **Замкнутость UX.** Каждый user journey имеет начало и конец. Нет «висящих» сценариев. Если сценарий передаёт control — есть ли получатель?
